...
> We use **Docker** to run the local MySQL database. You do not need XAMPP.
> If XAMPP is installed, keep it closed so it does not block port `3306`.
...
> Crucial: use `127.0.0.1` for `DB_HOST` (not `localhost`) to avoid Node.js IPv6 resolution issues.

```env
# Database — matches local Docker MySQL config
DB_HOST=127.0.0.1
DB_USER=root
DB_PASSWORD=
...
### 4. Start the local MySQL database (Docker)

Make sure Docker Desktop is running, then start MySQL in the background:

```bash
docker run -d --name msc-mysql -p 3306:3306 -e MYSQL_ALLOW_EMPTY_PASSWORD=yes mysql:8.0
```
...
→ Docker MySQL is not running. Start Docker Desktop, then run the `docker run ... mysql:8.0` command from Step 4.

**`Bind for 0.0.0.0:3306 failed: port is already allocated`**  
→ Port `3306` is already in use (often by XAMPP/MySQL). Stop that service, then run the Docker command again.

**`Error: listen EADDRINUSE :::5000`**  
→ Port 5000 is already taken. Change `PORT=5001` in `server/.env`.

**`ER_ACCESS_DENIED_ERROR`**  
→ Wrong DB credentials. For this setup, use `root` with empty password and confirm `.env` matches Step 3. If your server has a password, add your password in the field of the password.
